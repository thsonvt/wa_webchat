...
MySimpleCarousel.prototype.writeHTML = function() {
    ...
    // console.log('message.user_defined.slides ///// ' + JSON.stringify(message.user_defined.slides, null, 2))
    message.user_defined.locations.forEach(function(location) {
        content.innerHTML +=
            '<li><a target="_blank" href="' + location.link + '"><img class="carousel_slide_image" src="' + 
            location.image +
            '"/></a><div class="carousel_slide_title">' +
            location.state + ' ' + location.name + 
            '</div><div class="carousel_slide_description">' +
            '<b>Address</b>:' + location.address + ' <br/> <b>Operation hours</b>: ' + location.operation_hour
            '</div></li>';
    });
    this.event.data.element.appendChild(element);
};
...
function handleCarouselTemplate(event) {
  // console.log('handleCarouselTemplate -- event.date.message ' + JSON.stringify(event.data.message, null, 2))
